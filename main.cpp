...
#include"bank.h"
uint16 Global_uint8bankSystemMode 	     	= 0;									//Variable To Store the Mode of the system.
uint16 Global_uint8bankSystemModes_ON_OFF 	= 0;									//Variable To Store the Mode of the system.
uint16 Global_uint8bankSystemAdminOption  	= 0;									//Variable To Store The Option Entered By the Admin.
uint16 Global_uint8bankSystemUserOption  	= 0;									//Variable To Store The Option Entered By the Admin.
uint16 Global_uint8bankSystemAdminOPENOption = 0;	
...
int main(void)
{
	while(1)
	{
		...
		cin>>Global_uint8bankSystemMode;
				...
				/*Checking from the Global Variable that the Password checking is done successfully*/
				 if(BANK_SYSTEM_VALID == Global_uint8bankCheck)
				 {
					...
					/*Scaning the option Entered by the Admin*/
						 cin>>Global_uint8bankSystemAdminOption;
						 ...
						 switch(Global_uint8bankSystemAdminOption)
						 {
							...
							/********************************* BANK_SYSTEM_OPEN_EXISTING_ACCOUNT ****************************************/
							 ...
							 case BANK_SYSTEM_OPEN_EXISTING_ACCOUNT:
									LinkedList_Traverse();
									bank_id();
									cout<<"|********************* Welcome In Admin Mode BANK_SYSTEM_OPEN_EXISTING_ACCOUNT *******************|\n\
									...
									/*Scaning the option Entered by the Admin*/
									cin>>Global_uint8bankSystemAdminOPENOption;
									/*Scaning the option Entered by the Admin while it is invalid*/
				 ...
				 else
				 {
					/*Printing an Error message that the password Entring Trials Ran out*/	
					 cout<<"Sorry, You Used all the Trials. \n"<<endl;
				 }
					...
					/*User mode Activated till the User Press Exit from the options*/
					 while(BANK_SYSTEM_USER_MODE_ON == Global_uint8bankSystemModes_ON_OFF)
					 {
						...
						/*Scaning the option Entered by the User*/
						 cin>>Global_uint8bankSystemUserOption;
						...
						/*Switching on the Option Entered by the User*/
						 switch(Global_uint8bankSystemUserOption)
						 {
							...
							case BANK_SYSTEM_MAKE_TRANSACTION:
									...
									/*Calling Function to View a clients Record*/
									bankSystem_VoidAdminTransaction();
